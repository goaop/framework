...
- Remote session (Claude Code on the web): `composer install` is source-only/slow and dies on phpstan/phpstan (use the packagist mirror), Functional tests fail under PHP 8.5 JIT → recipes in `tests/AGENTS.md` § Remote sessions.
