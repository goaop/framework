...
* [BC BREAK] [Deprecation] `AspectContainer::hasAnyResourceChangedSince()` returned the opposite of its documented contract ("are there new changes") — it actually reported "everything is fresh". The freshness check now lives in the new `AspectContainer::isFreshSince(int $timestamp): bool`; `hasAnyResourceChangedSince()` returns what its docblock always promised (true when the modification time of any tracked resource is greater than the given timestamp) and is marked `#[\Deprecated]` — use `isFreshSince()` instead.
