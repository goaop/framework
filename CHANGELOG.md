...
* [Feature] **First-class callable advices, lazily constructed** — the main way advices are now wired into woven code. Generated proxies declare each aspect-method advice as static data, e.g. `Interceptor::before(MonitorAspect::class, 'beforeMethodExecution')`, and the interceptor comes back as a native PHP lazy proxy: the interceptor construction, the aspect resolution from the container and the creation of the first-class advice callable on the aspect instance are all deferred until the advice is actually used, so advices that never run never instantiate their aspect (`LazyAdvisorAccessor` is removed). Advices registered in the container as plain closures (not aspect methods) are resolved through the new `The::advice('advisorId')` accessor, which unwraps `Advisor` and interceptor values down to the raw advice closure.
