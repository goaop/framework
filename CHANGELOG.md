...
* [BC BREAK] The `Pointcut`, `Advice` and `Advisor` interfaces now extend the internal `Go\Aop\Compilable` contract: custom implementations must add `compileToPhp(): PhpParser\Node\Expr` emitting a static expression that recreates the instance for the compiled advisor cache. An item that cannot express itself statically (e.g. an advice closure not scoped to an `Aspect` class) throws `Go\Core\Cache\NotCompilableException` from the cache writer — such aspects cannot run with the advisor cache enabled; cache read/write errors in general now throw instead of raising warnings.
