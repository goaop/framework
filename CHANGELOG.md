...
* [Performance] [BC BREAK] **Engine-probed lazy-proxy compatibility (`Go\Core\NativeLazyProxy`)** — the container no longer decides per class in userland whether PHP can create a native lazy proxy (reflection over all properties plus a parent-chain walk on the service read path). The new shared helper simply asks the engine: `newLazyProxy()` throws for incompatible classes (memoized for long-running workers) and one `isUninitializedLazyObject()` call detects property-less classes, which keep the eager construction path. Generated interceptor code (`Interceptor::before(Aspect::class, 'method')` cache form) uses the same helper. This also retires the stale readonly-before-8.5 version gate: readonly service classes are lazy-proxyable wherever the running engine supports it (current PHP 8.4.x does).
* [BC BREAK] **Container throws SPL exceptions** — `Go\Core\Container` no longer throws `Go\Aop\AspectException`: a non-class lazy service id raises `InvalidArgumentException` and a service/factory result of the wrong type raises `UnexpectedValueException` (`OutOfBoundsException` for missing entries is unchanged).
* [BC BREAK] **Framework service wiring moved out of the container constructor** — `new Container()` registers nothing; the kernel registers the framework's own deferred services via the new `Go\Core\FrameworkServices::register($container)` during `init()`. Code constructing a bare `Container` for framework use must call it explicitly.
