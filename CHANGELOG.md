...
* [Performance] [BC BREAK] **Lazy transformation pipeline** — the stream filter and the source transformers are only registered/constructed on the first cache miss (`SourceTransformingLoader::ensureRegistered()`), never on a warm-cache request. The protected `AspectKernel::registerTransformers()` hook was replaced by `AspectKernel::createSourceTransformers()`, which returns the inner transformer chain; kernels overriding the old hook must migrate.
